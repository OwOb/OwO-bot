...
bot.on("message", function(message) {
  ...
  if(headlower == "!test") {
    ...
    */
    ...
    var mmm = safeEval("const {c, cpp, node, python, java} = require(\"compile-run\"); c.runSource(\""+message.content.substring("!a".length)+"\",{executionPath: \"python3\", stdin: \"\"}).then(result => {var resultmessage = result.stdout;}).catch(err => {\"QQ\\n\\n\"+err;});");
