    ...
    try {
      ...
      GoogleImagesClient.search(search).then(images => {
        if (images.length > 0) {
          ...
          var richembed = new Discord.RichEmbed().setColor(3447003).setTitle("**"+search.replace(/\\/g,"\\\\").replace(/\*/g,"\\*").replace(/~/g,"\\~").replace(/\_/g,"\\_").replace(/`/g,"\\`")+"**").setImage(images[index]["url"]).addField("",images[index]["url"]);
          message.channel.send(richembed);
        }
        else
